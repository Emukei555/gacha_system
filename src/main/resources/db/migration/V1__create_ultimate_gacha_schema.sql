...
CREATE TABLE gacha_pools (
                             ...
                             version BIGINT NOT NULL DEFAULT 0, -- 楽観ロック用
...
CREATE TABLE wallets (
                         user_id UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE, -- Userテーブルと紐付け
...
CREATE TABLE gacha_transactions (
                                    id UUID NOT NULL, -- UUID v7 (Idempotency Key)
...
-- 初期パーティション (自動化スクリプトは別途管理ツールで実行推奨だが、初期構築用に数ヶ月分作成)
